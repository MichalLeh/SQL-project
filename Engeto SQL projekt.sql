...
left join 
# Gini koeficient; inner join vynechá státy jako: Afghanistan, New Zealand, Saudi Arabia, Cuba, Singapore, Libya atd... a zároveò nemají Null hodnotu v ostatních zkoumaných promìnných
(select country, gini, max(year) from economies where gini is not Null group by country) ec3
on ctr.country = ec3.country
# left join zahrnuje rovnìž Russian Federation (144 mil. obyvatel)
left join 
